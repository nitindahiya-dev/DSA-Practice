...
//         // Push right first, so that left is processed first (pre-order)
//         if(head->right != NULL) {
//             st.push(head->right);
//         }
//         if(head->left != NULL) {
...
//     vector<int> result = preOrder(head);
...
//             // Process the current node (in-order visit)
//             curr = st.top();
//             st.pop();
...
//     vector<int> result = inOrder(head);
